...
from communication import insert_user, today_schedule, nextday_schedule, next_week_schedule, current_week_schedule, \
    get_current_weeknum, update_schedule, specify_week_schedule, user_schedule_on_exams, current_main_exams_schedule
...
@bot.message_handler(commands=['first_exams'])
def current_week(message: types.Message):
    try:
        res = ''
...
def current_week(message: types.Message):
    try:
        res = current_main_exams_schedule(message.from_user.id)
        bot.send_message(message.chat.id, res)
    except KeyError as e:
        bot.send_message(message.chat.id, 'Вы не зарегистрированы')
